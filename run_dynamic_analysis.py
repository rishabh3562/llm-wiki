...
from retry_utils import retry_subprocess, retry_github_api, retry_mongodb
...
@retry_subprocess(max_retries=2, base_delay=0.5)
...
def discover_github_repos(username, token):
    ...
    while True:
        ...
        for attempt in range(max_retries):
            ...
            # This else corresponds to the for loop, meaning we exhausted retries without breaking
            # The last exception was already raised in the loop, so we break out of the while
            break
...
def process_repo(repo_info, ledger, storage_success_global):
    ...
    # If initial analysis is already done, skip this repo entirely
    if repo_ledger.get("initial_analysis_done"):
        print(f"   ✅ Initial analysis already done. Skipping.")
        # Update queue status to done for skipped repos (they are already processed)
        try:
            ...
            print(f"   ⚠️  Failed to update queue status for skipped repo: {e}")
...
def store_snippets_mongodb(repo_name, head_sha, output_base):
    """Store snippets to MongoDB with retry logic."""
    ...
    if storage_success:
        try:
            ...
            # Update queue status after successful processing
            try:
                queue_client = MongoClient(MONGO_URI, tlsAllowInvalidCertificates=True, serverSelectionTimeoutMS=5000)
                queue_db = queue_client[MONGO_DB]
                queue_coll = queue_db["repo_queue"]
        ...
        # Update queue status to failed even if storage failed
        try:
            queue_client = MongoClient(MONGO_URI, tlsAllowInvalidCertificates=True, serverSelectionTimeoutMS=5000)
            queue_db = queue_client[MONGO_DB]
            queue_coll = queue_db["repo_queue"]
...
def main():
    ...
    # QUEUE STRATEGY: Process only ONE repo that needs initial analysis
    # Filter out repos where initial_analysis_done == true
    repos_needing_analysis = []
    for repo_info in updated_repos:
        repo_name = repo_info["name"]
        repo_ledger = ledger.get(repo_name, {})
        if not repo_ledger.get("initial_analysis_done"):
            repos_needing_analysis.append(repo_info)
    
    # Sort alphabetically by repo name
    repos_needing_analysis.sort(key=lambda x: x["name"])
    
    if not repos_needing_analysis:
        print(f'   🎉 All repositories have had initial analysis done!')
        # Still save ledger (though nothing changed)
        save_json(LEDGER_PATH, ledger)
        print(f"\n📋 RUN SUMMARY")
        print(f"   Repositories processed: 0")
        print(f"   Repositories skipped (no new commits): 0")
        print(f"   Repositories failed: 0")
        print(f"   Total snippets added to MongoDB: 0")
        print(f"   ✅ Updated ledger.json")
        print(f'   ℹ️  No repositories need initial analysis.')
        return
    
    # Take the first repo that needs analysis
    repo_info = repos_needing_analysis[0]
    print(f'   🎯 Selected repo for initial analysis: {repo_info["name"]}')
