...
def collect_repo_metrics(repo_path, smells):
    ...
    return {
        ...
        "top_level_dirs": [{"name": name, "count": count} for name, count in top_level_dirs.most_common(12)],
...
def compute_note_target(metrics):
    base = 6
    extra = 0
    extra += math.ceil(metrics["code_files"] / 10)
    extra += math.ceil(metrics["loc"] / 5000)
    extra += math.ceil(metrics["top_level_dir_count"] / 2)
    extra += math.ceil(len(metrics["largest_files"]) / 6)
    target = base + extra
    return max(6, min(40, target))
