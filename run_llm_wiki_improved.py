...
def process_repo(repo_info, ledger, storage_success_global):
    ...
    repo_path_str = str(local_path)
    if not (local_path / ".git").exists():
        print(f"   Cloning repository...")
        clone_url = repo_info["html_url"]
        clone_result = run_cmd(f"git clone {clone_url} .", cwd=local_path)
        if clone_result[2] != 0:
            print(f"      ❌ Clone failed: {clone_result[1]}")
            return False, 0
    else:
        print(f"   Fetching latest changes...")
        run_cmd("git fetch origin", cwd=local_path)
        # Reset to origin/HEAD (default branch)
        run_cmd("git reset --hard origin/HEAD", cwd=local_path)
